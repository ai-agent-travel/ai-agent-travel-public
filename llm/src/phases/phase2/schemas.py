...
class HotelSearch(BaseModel):
    area: AreaCode = Field(..., description="地域")
    checkin: str = Field(..., description="チェックイン日")
    checkout: str = Field(..., description="チェックアウト日")
    budget: int = Field(..., description="予算")
    num_adults: int = Field(..., description="人数")
...
class RestaurantSearch(BaseModel):
    ...
    budget: int = Field(..., description="予算")
    cuisine_type: str = Field(..., description="料理ジャンル")
    num_people: int = Field(..., description="人数")

class SpotSearch(BaseModel):
    area: str = Field(..., description="地域")
    category: str = Field(..., description="カテゴリ")
    estimated_hours: float = Field(..., description="観光時間")

class SearchTask(BaseModel):
    hotel_search: List[HotelSearch] = Field(..., description="ホテル検索タスク")
    restaurant_searches: List[RestaurantSearch] = Field(..., description="レストラン検索タスク")
    spot_searches: List[SpotSearch] = Field(..., description="観光スポット検索タスク")

class TaskList(BaseModel):
    tasks: List[SearchTask] = Field(..., description="タスクリスト")
